...
import type { HisenseData, Datadik, Ptk } from '@/context/AppProvider';
import { useDraggable } from '@/hooks/useDraggable';
import Fuse from 'fuse.js';

interface StickyInfoBoxProps {
  hisenseData: HisenseData;
  datadikData: Datadik;
}
...
export default function StickyInfoBox({ hisenseData, datadikData }: StickyInfoBoxProps) {
  ...
  const ptkList = datadikData.ptk || [];

  const isAddressMatch = cleanAndCompare(hisenseData.schoolInfo.Alamat, datadikData.address) &&
    cleanAndCompare(hisenseData.schoolInfo.Kecamatan, datadikData.kecamatan) &&
    cleanAndCompare(hisenseData.schoolInfo.Kabupaten, datadikData.kabupaten);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontWeight: 'bold', fontSize: '15px', color: '#0056b3' }}>{hisenseData.schoolInfo.Nama}</div>
        <div style={{ fontSize: '12px', color: '#555' }}>NPSN: {hisenseData.schoolInfo.NPSN}</div>
      </div>

      <div style={{ padding: '12px 18px' }}>
        <div style={{ fontWeight: 'bold', fontSize: '13px', color: '#555' }}>Serial Number</div>
        <div style={{ fontSize: '16px', fontWeight: 600, color: '#000' }}>{hisenseData.schoolInfo['Serial Number']}</div>
        
        <div style={{ marginTop: '12px', border: '1px solid #ccc', borderRadius: '4px', padding: '8px' }}>
          <div style={{ fontSize: '12px', fontWeight: 'bold', marginBottom: '4px' }}>
            Alamat (Hisense & Datadik)
          </div>
          <div style={{ fontSize: '12px', color: '#333', lineHeight: '1.4' }}>
            <b>Hisense:</b> {hisenseData.schoolInfo.Alamat}, {hisenseData.schoolInfo.Kecamatan}, {hisenseData.schoolInfo.Kabupaten}
          </div>
          <div style={{ fontSize: '12px', color: '#333', lineHeight: '1.4' }}>
            <b>Datadik:</b> {datadikData.address}, {datadikData.kecamatan}, {datadikData.kabupaten}
          </div>
        </div>

        <div style={{ marginTop: '8px', fontSize: '12px', fontWeight: 'bold', color: '#0056b3' }}>
          <b>Kepala Sekolah:</b> {datadikData.kepalaSekolah}
        </div>
